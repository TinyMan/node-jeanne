...
const search = {
    ...
    exec: function(data) {
        ...
        ytapi().search(terms, this.config.extensions.youtube.search_max_results, (err, res) => {
            if (err) throw err;
            const breakTag = "<br>"
...
const youtube = {
    ...
    exec: function(data) {
        ...
        if (!links.length) {
            ...
            ytapi().search(data.message, 1, function(err, res) {
                ...
                if (res.items.length > 0)
                    streamYt(video_link(res.items[0].id.videoId), res.items[0]);
                else {
        ...
        function streamYt(link, item = null) {
            if (!item) item = {
                id: {
                    ...
                    format: {
                        ...
                        if (youtube.playlist.length > 0) {
                            ...
                            ytapi().related(id, 6, function(err, res) {
                                ...
                                if (item)
                                    streamYt(video_link(item.id.videoId), item);
                            })
                        }
                    }
                });
...
const youtubeInfo = {
    ...
    exec: function(data) {
        if (data.id) {
            ...
            ytapi().getById(id, (err, res) => {
                if (err) throw err
                res = res.items[0]
                const breakTag = "<br>"
                let message = "Now playing <b>" + res.snippet.title + "</b> (" + link_with_title(link) + "): " + breakTag;
                message += res.snippet.description.replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)+/g, '$1' + breakTag + '$2').autoLink();
