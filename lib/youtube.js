		...
		return getById(id)
			...
			.then(info => {
				...
				let format = chooseFormat(info.formats, {
					quality: 'highest',
					filter: this.startTime !== 0 ? "audio" : "audioonly" // hack to make the begin parameter usefull (see https://github.com/fent/node-ytdl-core/issues/129)
				})
				if (format.profile !== "high" && this.startTime !== 0) {
					format = chooseFormat(info.formats, { quality: 'highest', filter: 'audioonly' })
				}
				this.stream = ytdl.downloadFromInfo(info, {
					format,
					begin: format.profile === "high" ? this.startTime : 0
				})
				this.stream.on('error', e => { logger.error(e), this.emit('error', e) })
				this.stream.on('abort', e => { logger.error(e), this.emit('abort', e) })
				this.streamer.stream(this.stream, format.profile === "high" ? 0 : (this.startTime / 1000).toFixed(2))
			})
	}
	stop() {
