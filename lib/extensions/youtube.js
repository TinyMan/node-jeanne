		...
		this.space.get('youtube-player').search(terms)
			.then(res => {
				...
				for (let i in res.items) {
					...
					message += i + ". " + link_with_title(video_link(id), title) + ', <b>' + channelTitle + '</b>' + breakTag
				}
...
module.exports = {
	...
	commands: [play, stop, next, add, addnext, search, description, seek, jumpForward, jumpBackward, trending, previous, related]
}
